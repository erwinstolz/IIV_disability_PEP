...
# make line plots for the first 100 individuals
...
round(Weights(AICc(m1.lmer, m2.lmer, m3.lmer, m4.lmer)), 3) # will remain with
...
library(splines)
# with 3 knots
m6.lmer <- lmer(N_dis12 ~  ns(time.to.death.yrs.c, df=5) +
             age.b.70 + num.int.c + 
             (1 + time.to.death.yrs.c | id), 
           pep, REML=F)
summary(m6.lmer)
...
library(splines)
# with 3 knots
m8.lmer <- lmer(N_dis12 ~  ns(time.to.death.yrs.c, df=5) +
             age.b.70 + num.int.c + 
             (1 + ns(time.to.death.yrs.c, df=5) | id), 
           pep, REML=F)
summary(m8.lmer)
...
```{r}
# adding covariates
m9.lmer <- lmer(N_dis12 ~  time.to.death.yrs.c + age.dead.c + 
...
library(stargazer)
stargazer(m9.lmer, title=c("Disability trajectories"), 
